...
class PlanController extends Controller
{
    ...
    public function store(Request $request)
    {
        if(\Auth::user()->type == 'super admin')
        {
            if((env('ENABLE_STRIPE') == 'on' && !empty(env('STRIPE_KEY')) && !empty(env('STRIPE_SECRET'))) || (env('ENABLE_PAYPAL') == 'on' && !empty(env('PAYPAL_CLIENT_ID')) || !empty(env('PAYPAL_SECRET_KEY'))) || $request->price <= 0)
            {
                $validation                = [];
    ...
    public function update(Request $request, $plan_id)
    {
        if(\Auth::user()->type == 'super admin')
        {
            ...
            if(((env('ENABLE_STRIPE') == 'on' && !empty(env('STRIPE_KEY')) && !empty(env('STRIPE_SECRET'))) || (env('ENABLE_PAYPAL') == 'on' && !empty(env('PAYPAL_CLIENT_ID')) || !empty(env('PAYPAL_SECRET_KEY')))) || $plan->price <= 0)
            {

...
@section('title')
    <div class="d-inline-block">
        <h5 class="h4 d-inline-block font-weight-400 mb-0 ">{{__('Invoice Analytic')}}</h5>
    </div>
@endsection
@section('breadcrumb')
    <li class="breadcrumb-item"><a href="{{ route('dashboard') }}">{{__('Home')}}</a></li>
    <li class="breadcrumb-item active" aria-current="page">{{__('Invoice Analytic')}}</li>
@endsection
@section('action-btn')
@endsection
@section('content')
                ...
                <div class="cardcard-body">
                    <div class="collapse show float-right" id="collapseExample" style="">
                        ...
                        <div class="row filter-css">
                            ...
                            <div class="col-auto">
                                {{ Form::select('account',$account,isset($_GET['account'])?$_GET['account']:'', array('class' => 'form-control ','data-toggle'=>'select')) }}
                            </div>
                            <div class="col-auto">
                            ...
                            <div class="col-auto pt-2">
                                <a href="{{route('report.invoiceanalytic')}}" data-toggle="tooltip" data-title="{{__('Reset')}}" class="btn btn-xs btn-danger btn-icon-only rounded-circle"><i class="fas fa-trash-restore"></i></a>
                            </div>
                            <div class="col-auto pt-2">
                ...
                <table class="table align-items-center" id="reportTable">
                    ...
                    @foreach($invoice as $result)
                        <tr>
                            ...
                            <td>
                                @if($result->status == 0)
                                    <span class="badge badge-info">{{ __(\App\Invoice::$status[$result->status]) }}</span>
                                @elseif($result->status == 1)
                                    <span class="badge badge-info">{{ __(\App\Invoice::$status[$result->status]) }}</span>
                                @elseif($result->status == 2)
                                    <span class="badge badge-info">{{ __(\App\Invoice::$status[$result->status]) }}</span>
                                @elseif($result->status == 3)
                                    <span class="badge badge-success">{{ __(\App\Invoice::$status[$result->status]) }}</span>
                                @elseif($result->status == 4)
                                    <span class="badge badge-warning">{{ __(\App\Invoice::$status[$result->status]) }}</span>
                                @elseif($result->status == 5)
                                    <span class="badge badge-danger">{{ __(\App\Invoice::$status[$result->status]) }}</span>
                                @endif
                            </td>
                            <td class="">

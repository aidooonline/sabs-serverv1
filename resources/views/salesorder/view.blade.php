...
@endsection
@section('action-btn')
    <a href="{{route('salesorder.pdf',\Crypt::encrypt($salesOrder->id))}}" target="_blank" class="btn btn-sm btn-primary btn-icon rounded-pill">
        <span class="btn-inner--icon text-white"><i class="fa fa-print"></i></span>
        <span class="btn-inner--text text-white">{{__('Print')}}</span>
    </a>
    @can('Edit SalesOrder')
        <a href="{{ route('salesorder.edit',$salesOrder->id) }}" class="btn btn-sm btn-primary bor-radius" data-title="{{__('Sales order Edit')}}"><i class="far fa-edit"></i>
        </a>
    @endcan
    <a href="#" data-size="lg" data-url="{{ route('salesorder.salesorderitem',$salesOrder->id) }}" data-ajax-popup="true" data-title="{{__('Create New SalesOrder')}}" class="btn btn-sm btn-primary bor-radius">
                        ...
                        <div class="col-12">
                            <div class="row align-items-center mb-5">
                                <div class="col-sm-6 mb-3 mb-sm-0">
                                    {{--                                    <img src="{{asset("assets/img/brand/dark.png")}}" alt="" height="30">--}}
                                </div>
                                <div class="col-sm-6 text-sm-right">
                                    <h6 class="d-inline-block m-0 d-print-none">{{__('SalesOrder ')}}</h6>
                            ...
                            <div class="row mb-5">
                                <div class="col-lg-6 col-md-8">
                                    ...
                                    <span class="col-sm-8"><span class="text-sm">{{\Auth::user()->dateFormat($salesOrder->date_salesorderd)}}</span></span>
                                </div>
                            </div>
                                    ...
                                    <div class="table-responsive">
                                        <table class="table mb-0">
                                            ...
                                            @foreach($salesOrder->items as $salesOrderitem)
                                                ...
                                                @endphp
                                                <tr>
                                                    <td class="px-0">{{!empty($salesOrderitem->items->name)?$salesOrderitem->items->name:'--'}} </td>
                                                    <td class="px-0">{{$salesOrderitem->quantity}} </td>
                                                    <td class="px-0">{{\Auth::user()->priceFormat($salesOrderitem->price)}} </td>
                                                    <td class="px-0">
                                                        <div class="col">
                                                    ...
                                                    <td class="px-0">{{\Auth::user()->priceFormat($salesOrderitem->discount)}} </td>
                                                    <td class="px-0">{{!empty($salesOrderitem->description)?$salesOrderitem->description:'--'}} </td>
                                                    <td class="text-right"> {{\Auth::user()->priceFormat($salesOrderitem->price*$salesOrderitem->quantity)}}</td>
                                                    <td class="text-right">
                                                        @can('Edit SalesOrder')
                                                            <a href="#" data-url="{{ route('salesorder.item.edit',$salesOrderitem->id) }}" data-ajax-popup="true" class="action-item" data-toggle="tooltip" data-original-title="{{__('Edit')}}" data-title="{{__('Edit Item')}}"><i class="far fa-edit"></i></a>
                                                        @endcan
                                            ...
                                            <tr>
                                                ...
                                                <td class="text-right subTotal">{{\Auth::user()->priceFormat($salesOrder->getSubTotal())}}</td>
                                                <td></td>
                                            </tr>
                                            <tr>
                                                <td colspan="4">&nbsp;</td>
                                                ...
                                                <td class="text-right subTotal">{{\Auth::user()->priceFormat($salesOrder->getTotalDiscount())}}</td>
                                                <td></td>
                                            </tr>
                                            @if(!empty($taxesData))
                                            ...
                                            <tr>
                                                ...
                                                <td class="text-right subTotal">{{\Auth::user()->priceFormat($salesOrder->getTotal())}}</td>
                                                <td></td>
                                            </tr>
                                            </tfoot>
                                            </tbody>
                                            ...
                                            <div class="row justify-content-between align-items-center">
                                                <div class="col-md-6 order-md-2 mb-4 mb-md-0">
                                                    <div class="d-flex align-items-center justify-content-md-end">
                                                        ...
                                                        <span class="h4 mb-0">{{\Auth::user()->priceFormat($salesOrder->getTotal())}}</span>
                                                    </div>
                                                </div>
                            ...
                            <div class="row">
                                ...
                                <div class="col-12 col-md-4">
                                    ...
                                    <dl class="row mt-4 align-items-center">
                                        ...
                                        <dd class="col-sm-8"><span class="text-sm">{{ $salesOrder->billing_address }}</span></dd>

                                        <dt class="col-sm-4"><span class="h6 text-sm mb-0">{{__('Billing City')}}</span></dt>
                                        <dd class="col-sm-8"><span class="text-sm">{{ $salesOrder->billing_city }}</span></dd>

                                        <dt class="col-sm-4"><span class="h6 text-sm mb-0">{{__('Billing Country')}}</span></dt>
                                        <dd class="col-sm-8"><span class="text-sm">{{ $salesOrder->billing_country }}</span></dd>

                                        <dt class="col-sm-4"><span class="h6 text-sm mb-0">{{__('Zip Code') }}</span></dt>
                                        <dd class="col-sm-8"><span class="text-sm">{{ $salesOrder->billing_postalcode }}</span></dd>

                                        <dt class="col-sm-4"><span class="h6 text-sm mb-0">{{__('Billing Contact')}}</span></dt>
                                        <dd class="col-sm-8"><span class="text-sm">{{ !empty($salesOrder->contacts->name)?$salesOrder->contacts->name:'--'}}</span></dd>
                                    </dl>
                                </div>
                                <div class="col-12 col-md-4">
                                    ...
                                    <dl class="row mt-4 align-items-center">
                                        ...
                                        <dd class="col-sm-8"><span class="text-sm">{{ $salesOrder->shipping_address }}</span></dd>

                                        <dt class="col-sm-4"><span class="h6 text-sm mb-0">{{__('Shipping City')}}</span></dt>
                                        <dd class="col-sm-8"><span class="text-sm">{{ $salesOrder->shipping_city }}</span></dd>

                                        <dt class="col-sm-4"><span class="h6 text-sm mb-0">{{__('Shipping Country')}}</span></dt>
                                        <dd class="col-sm-8"><span class="text-sm">{{ $salesOrder->shipping_country }}</span></dd>

                                        <dt class="col-sm-4"><span class="h6 text-sm mb-0">{{__('Zip Code')}}</span></dt>
                                        <dd class="col-sm-8"><span class="text-sm">{{ $salesOrder->shipping_postalcode }}</span></dd>
